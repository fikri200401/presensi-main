...
<!-- Sub-header: Manajemen Jadwal Kerja -->
<div class="mb-5 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
    <h3 class="text-lg font-semibold text-gray-900">Manajemen Jadwal Kerja</h3>
...
<div class="mb-5 flex flex-col sm:flex-row gap-3">
    <form method="GET" action="{{ route('schedule.index') }}" class="flex gap-2 flex-1">
        ...
        <table class="min-w-full divide-y divide-gray-100">
            <thead>
                <tr class="bg-gray-50/50">
                    ...
                    <th class="px-6 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Kantor</th>
            ...
            <tbody class="bg-white divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50/50 transition-colors">
                    <td class="px-6 py-3.5 whitespace-nowrap">
                        <div class="flex items-center gap-3">
                            ...
                            <p class="text-sm font-semibold text-gray-900">{{ $schedule->user->name ?? 'N/A' }}</p>
                        </div>
                    </td>
                    <td class="px-6 py-3.5 whitespace-nowrap text-sm text-gray-900">{{ $schedule->shift->name ?? 'N/A' }}</td>
                    <td class="px-6 py-3.5 whitespace-nowrap text-sm text-gray-500">{{ $schedule->office->name ?? 'N/A' }}</td>
                    <td class="px-6 py-3.5 whitespace-nowrap">
                        <div class="flex gap-1">
                            @if($schedule->is_wfa)
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-700">WFA</span>
                            @endif
                            @if($schedule->is_banned)
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-50 text-red-700">Banned</span>
                            @endif
                            @if(!$schedule->is_wfa && !$schedule->is_banned)
                    ...
                    <td class="px-6 py-3.5 whitespace-nowrap text-center">
                        <div class="flex items-center justify-center gap-2">
                            <a href="{{ route('schedule.edit', $schedule) }}" class="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                            </a>
                            <form method="POST" action="{{ route('schedule.destroy', $schedule) }}" class="inline" onsubmit="return confirm('Yakin hapus jadwal ini?')">
                                @csrf
                                @method('DELETE')
                ...
                <tr>
                    <td colspan="5" class="px-6 py-12 text-center">
                        ...
                        <p class="text-sm text-gray-400">Tidak ada data jadwal ditemukan</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
