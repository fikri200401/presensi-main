            ...
            <div>
                ...
                @if(auth()->user()->hasRole(['super_admin', 'admin']))
                    <select id="status" name="status" required
                        ...
                        <option value="approved" {{ old('status', $leave->status) == 'approved' ? 'selected' : '' }}>Disetujui</option>
                        <option value="rejected" {{ old('status', $leave->status) == 'rejected' ? 'selected' : '' }}>Ditolak</option>
                    </select>
                @else
                    <input type="text" value="{{ ucfirst($leave->status) }}" readonly
                           class="block w-full rounded-xl border border-gray-200 bg-gray-100 px-3.5 py-2.5 text-sm text-gray-500 cursor-not-allowed">
                    <input type="hidden" name="status" value="{{ $leave->status }}">
                    @if(in_array($leave->status, ['approved', 'rejected']))
                        <p class="mt-1 text-xs text-gray-500">Pengajuan ini telah {{ $leave->status == 'approved' ? 'disetujui' : 'ditolak' }}. Tidak dapat diubah.</p>
                    @else
                        <p class="mt-1 text-xs text-gray-500">Status akan tetap pending hingga ditinjau admin.</p>
                    @endif
                @endif
